...
public class AuthService {
    ...
    public ResponseEntity login(LoginReqDto loginReqDto){
        try {
            ...
            return ResponseBody.sendBody(ResponseMessages.LOGIN_FAILED(), null, HttpStatus.BAD_REQUEST);
        }catch (Exception e){
    ...
    public ResponseEntity registerUser(RegisterReqDto registerReqDto){
        try{
            ...
            if(newUser.getEmail() == null || newUser.getPassword() == null || newUser.getFirstName() == null || newUser.getLastName() == null){
                return ResponseBody.sendBody(ResponseMessages.USER_REGISTER_DATA_NOT_FOUND(), null, HttpStatus.BAD_REQUEST);
            }
            ...
            return ResponseBody.sendBody(ResponseMessages.REGISTER_SUCCESS(), token, HttpStatus.OK);
        }catch (Exception e){
            return ResponseBody.send(e.getMessage(), null, HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    public ResponseEntity confirmUser(String token){
        try {
            ...
            if(userConfirmDto.getActivation_code() == null || userConfirmDto.getUser_id() == null){
                return ResponseBody.sendBody(ResponseMessages.USER_CONFIRMATION_DATA_NOT_FOUND(), null, HttpStatus.BAD_REQUEST);
            }
